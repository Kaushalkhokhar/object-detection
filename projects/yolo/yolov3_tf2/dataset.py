...
@tf.function
def transform_targets_for_output_coco(y_true, grid_size, anchor_idxs):
    # y_true: (N, boxes, (x1, y1, x2, y2, class, best_anchor))
    N = tf.shape(y_true)[0]
    ...
    for i in tf.range(N):
        for j in tf.range(tf.shape(y_true)[1]):
            ...
            if tf.reduce_any(anchor_eq):
                ...
                anchor_idx = tf.cast(tf.where(anchor_eq), tf.int32)
                grid_xy = tf.cast(box_xy // (1/grid_size[0]), tf.int32)
...
def transform_targets_coco(y_train, anchors, anchor_masks, resize):
    y_outs = []
    grid_size = (resize[0] // 32, resize[1] // 32)
    ...
    for anchor_idxs in anchor_masks:
        y_outs.append(transform_targets_for_output_coco(
            y_train, grid_size, anchor_idxs))
        grid_size *= 2
